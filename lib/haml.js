...
window.haml = {
    ...
    while (!tokeniser.token.eof) {
      if (!tokeniser.token.eol) {
        ...
        if (tokeniser.token.exclamation) {
          haml.ignoredLine(tokeniser, indent, outputBuffer);
        } else if (tokeniser.token.equal) {
          haml.embeddedJs(tokeniser, indent, outputBuffer);
        } else {
  ...
  ignoredLine: function (tokeniser, indent, outputBuffer) {
    if (tokeniser.token.exclamation) {
  ...
  embeddedJs: function (tokeniser, indent, outputBuffer) {
    if (tokeniser.token.equal) {
      var currentParsePoint = tokeniser.currentParsePoint();
      var contents = tokeniser.skipToEOLorEOF();
      outputBuffer.append(haml.indentText(indent));
      outputBuffer.flush();
      outputBuffer.appendToOutputBuffer('    html += haml.evaluateExpression(context, "' +
        haml.escapeJs(contents) + '", ' +
        currentParsePoint.lineNumber + ', ' + currentParsePoint.characterNumber + ', "' +
        haml.escapeJs(currentParsePoint.currentLine) + '", true) + "\\n";\n');
    }
  },
  ...
  evaluateExpression: function (context, expression, lineNumber, characterNumber, currentLine, escapeHtml) {
    try {
      ...
      if (!_.includes(expression, 'return ')) {
