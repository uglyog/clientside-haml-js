...
root.haml = {
  ...
  compileHaml: function (templateId) {
    ...
    var result = this.compileHamlToJs(new haml.Tokeniser({templateId: templateId}), new haml.JsCodeGenerator());
    var fn = null;
  ...
  compileStringToJs: function (string) {
    var result = this.compileHamlToJs(new haml.Tokeniser({template: string}), new haml.JsCodeGenerator());
    var fn = null;
  ...
  compileHamlToJsString: function (string) {
    ...
    result += this.compileHamlToJs(new haml.Tokeniser({template: string}), new haml.JsCodeGenerator());
    result += '}\n';
    return result;
  },
  ...
  templateLine: function (tokeniser, elementStack, indent, generator) {
    ...
    var contents = haml.elementContents(tokeniser, indent + 1, tagOptions, generator);
  ...
  openElement: function (currentParsePoint,
    ...
    if (attributeHash.length > 0 || objectRef.length > 0) {
      generator.generateCodeForDynamicAttributes(id, classes, attributeList, attributeHash, objectRef,
        currentParsePoint);
    } else {
